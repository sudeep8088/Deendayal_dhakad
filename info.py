...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002250676177').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002384704536'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002384704536'))  # Bin channel id (make sure bot is admin)
...
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/ModijiURL_Open/7')  # Tutorial video link for opening shortlink website
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002372221620')).split()]
